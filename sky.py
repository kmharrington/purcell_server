...
class sky_point():
    ...
    def getLoc(self, unit = RADC):
        self.location.compute(self.telescope)
        if unit == sky_point.RADC:
            return (angle2tuple(self.location.ra), angle2tuple(self.location.dec))
        elif unit == sky_point.GALACTIC:
            ga = ep.Galactic(self.location)
            return (angle2tuple(ga.lon), angle2tuple(ga.lat))
        elif unit == sky_point.AZEL:
            return (angle2tuple(self.location.az), angle2tuple(self.location.alt))
        elif unit == sky_point.HADC:
    ...
    def __str__(self):
        self.location.compute(self.telescope)
        return 'Location At RA:' + str(self.location.ra) + ' , DEC:' + str(self.location.dec) + '\nAZ: ' + str(self.location.az) + ' , EL: ' + str(self.location.alt)
