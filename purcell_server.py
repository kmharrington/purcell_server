    ...
    def process_response(self,data):
        ...
        if data[0] == 'move':
            ...
            if cmd[0] == 'EXECUTE':
                ...
                (ha,dc) = self.server.tel.clicks2hadc(self.server.tel.currentMoveGoal)
            ...
            if self.receiver:
                self.receiver.push(s.LimitResponse(self.server.tel.getLimitStatus()) + '\n')
        elif data[0] == 'error':
...
class proxy_receiver (asynchat.async_chat):
    ...
    '''
    channel_counter = 0
    def __init__ (self, server, sender, (conn, addr)):
    ...
    def moveCommand(self, cmd):
        ...
        self.server.tel.setLocGoal(self.server.tel.hadc2clicks( (new_ha,new_dec) ))
...
class telescope():
    ...
    Contains
        (eq_clicks, dc_clicks) -- The current location of the telescope, not updated
                                until a move has been completed
        (eq_goal, dc_goal)     -- The most recent location the client sent a request
                                to move to
        status - MOVING or IDLE. Telescope is only idle if it is which a minimum step
    ...
    def hadc2clicks(self, (ha, dec)):
        ha_clicks = telescope.CLKS_PER_DEG * (ha[0] +ha[1]/60.0 + ha[2]/3600.0)
        dec_clicks = telescope.CLKS_PER_DEG * (dec[0] +dec[1]/60.0 + dec[2]/3600.0) 
        return (int(ha_clicks), int(dec_clicks))
    
    def clicks2hadc(self, (ha_clicks, dec_clicks)):
    ...
    #Only call updateStatus when you're willing to send a move command to the telescope
    def updateStatus(self, sender):
        #We only move Eq OR Dc b/c we don't want to pile up commands on the Arduino
        if self.moveEQ(sender):#returns true if we don't have to move Eq
            if self.moveDC(sender):#returns true if we don't have to move DC
                self.status = telescope.IDLE
            else:
